// Document Expiration Cron Job
// This script should be run daily to check for expiring documents and send reminders
// Can be deployed as a Netlify function or run via a cron service
...
async function sendBulkQuarterlyReminder() {
  try {
    ...
    if (driversError) {
      console.error('Error fetching all active drivers:', driversError);
    ...
    console.log(`Bulk reminder completed: ${driversNotified}/${totalDrivers} drivers notified`);
    return {
      total_drivers: totalDrivers,
      drivers_notified: driversNotified,
      drivers_with_expiring_docs: driversWithExpiringDocs,
...
async function main() {
  try {
    // Always check for expiring documents
    ...
    if (shouldSendQuarterlyReminder()) {
      ...
      await sendBulkQuarterlyReminder();
    }
    
    console.log('All reminder tasks completed successfully');
  } catch (error) {
